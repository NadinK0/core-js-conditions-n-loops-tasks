...
function canQueenCaptureKing(queen, king) {
  let res = false;
  const diag1 = Math.abs(queen.x - king.x);
  const diag2 = Math.abs(queen.y - king.y);
  if (queen.x === king.x || queen.y === king.y || king.x === king.y) {
    res = true;
  } else if (diag1 === diag2) {
    res = true;
  }
...
function isIsoscelesTriangle(a, b, c) {
  let res = false;
  if (a === b && a * a - (c / 2) * (c / 2) > 0 && c !== 0) {
    res = true;
  } else if (a === c && a * a - (b / 2) * (b / 2) > 0 && b !== 0) {
    res = true;
  } else if (b === c && b * b - (a / 2) * (a / 2) > 0 && a !== 0) {
    res = true;
  }
  return res;
}
...
function isPalindrome(str) {
  ...
  const arr2 = [];
  if (str.length % 2) {
    for (let i = l + 1; i < arr.length; i += 1) {
      arr2[i - l - 1] = arr[i];
    }
  } else {
...
function shuffleChar(str, iterations) {
  ...
  let resStr = sortStr(str);
  let n = iterations - 1;

  while (n > 0 && resStr !== str) {
    resStr = sortStr(resStr);
    n -= 1;
  }
  let k = iterations % (iterations - n);
  while (k > 0) {
    resStr = sortStr(resStr);
    k -= 1;
  }
